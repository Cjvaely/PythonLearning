...
class face_emotion():
    def __init__(self):
        # 使用dlib库提供的特征提取器get_frontal_face_detector
        self.detector = dlib.get_frontal_face_detector()
        # dlib库提供的人脸68个特征点的模型
        self.predictor = dlib.shape_predictor("./shape_predictor_68_face_landmarks.dat")

        # 建立cv2摄像头对象，这里使用开发板的USB摄像头
        self.cap = cv2.VideoCapture(0)
        # 设置视频参数，3表示视频每帧的宽度，4表示高度，480是设置的大小值
        self.cap.set(3, 480)
        # 截图screen shoot的计数器
        self.cnt = 0
    ...
    def learning_face(self):
        ...
        while(self.cap.isOpened()):
            ...
            if(len(faces) != 0):
                ...
                for i in range (len(faces)):
                    ...
                    for k, d in enumerate(faces):
                        ...
                        # print("嘴巴高度与识别框高度之比：",mouth_height_arv)
                        ...
                        eye_height = (eye_sum / 4) / self.face_width
                        ...
                        if round(mouth_height >= 0.03):
                            if eye_height >= 0.052:
                                ...
                                cv2.putText(im_rd, "happy", (d.left(), d.bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX,
                                             0.8, (0, 0, 255), 2, 4)
                        # 没有张嘴，可能是正常和生气
                        else:
                            if self.brow_k <= -0.3:
                                cv2.putText(im_rd, "angry", (d.left(), d.bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX,
                                            0.8, (0, 0, 255), 2, 4)
                            else:
                                cv2.putText(im_rd, "nature", (d.left (), d.bottom () + 20), cv2.FONT_HERSHEY_SIMPLEX,
                                            0.8, (0, 0, 255), 2, 4)
                # 标出人脸数
            ...
            if(key == ord('q')):
                break
            # 窗口显示
            cv2.imshow("EmotionCamera", im_rd)
        # 释放摄像头
        self.cap.release()
